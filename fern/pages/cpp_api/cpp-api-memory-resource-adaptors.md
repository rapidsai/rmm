...
### Failure Callback Resource Adaptor

Construct a new `failure_callback_resource_adaptor` using `upstream` to satisfy
...
### Limiting Resource Adaptor

Resource that uses an upstream resource to allocate memory and limits the total
...
Construct a new limiting resource adaptor using `upstream` to satisfy
...
### Logging Resource Adaptor

Resource that uses an upstream resource to allocate memory and logs information
...
Construct a new logging resource adaptor using `upstream` to satisfy
...
Construct a new prefetch resource adaptor using `upstream` to satisfy
...
### Statistics Resource Adaptor

Resource that uses an upstream resource to allocate memory and tracks allocation
...
Construct a new thread safe resource adaptor using `upstream` to satisfy
...
Default constructor creates an allocator using the default memory

```cpp
RMM_EXEC_CHECK_DISABLE thrust_allocator() {} * @brief Constructs a `thrust_allocator` using the default device memory * resource and specified stream. * * @param stream The stream to be used for device memory (de)allocation */ RMM_EXEC_CHECK_DISABLE
```

_Source: `cpp/include/rmm/mr/thrust_allocator_adaptor.hpp:65`_

### Thrust Allocator

Constructs a `thrust_allocator` using the default device memory

```cpp
RMM_EXEC_CHECK_DISABLE explicit thrust_allocator(cuda_stream_view stream) : _stream{stream} {} * @brief Constructs a `thrust_allocator` using a device memory resource and * stream. * * @param mr The resource to be used for device memory allocation * @param stream The stream to be used for device memory (de)allocation */
```

_Source: `cpp/include/rmm/mr/thrust_allocator_adaptor.hpp:74`_

### Thrust Allocator

Constructs a `thrust_allocator` using a device memory resource and
...
```cpp
RMM_EXEC_CHECK_DISABLE thrust_allocator(thrust_allocator&& other) noexcept : Base(std::move(other)), _stream{other._stream}, _mr(std::move(other._mr)), _device{other._device} {
```
...
Copy constructor from a `thrust_allocator` of a different type.
