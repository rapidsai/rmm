...
struct MRTest : public ::testing::Test {
  ...
  void test_mixed_random_allocation_free_base(std::size_t max_size = 5_MiB,
                                              cudaStream_t stream  = 0);
...
void MRTest<MemoryResourceType>::test_random_allocations(std::size_t num_allocations,
                                                         cudaStream_t stream)
{
  return test_random_allocations_base(num_allocations, 5_MiB, stream);
}
...
{
  test_mixed_random_allocation_free_base(5_MiB, stream);
}
