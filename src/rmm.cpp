...
const char *
rmmGetErrorString(rmmError_t errcode) {
  switch (errcode) {
...
rmmError_t
rmmInitialize(rmmOptions_t *options) {
...
bool
rmmIsInitialized(rmmOptions_t *options) {
  if (nullptr != options) { *options = rmm::Manager::getOptions(); }
  return rmm::Manager::getInstance().isInitialized();
}

// Allocate memory and return a pointer to device memory.
rmmError_t
rmmAlloc(void **ptr, size_t size, cudaStream_t stream, const char *file, unsigned int line) {
...
rmmError_t
rmmFree(void *ptr, cudaStream_t stream, const char *file, unsigned int line) {
  return rmm::free(ptr, stream, file, line);
}

// Get the offset of ptr from its base allocation
rmmError_t
rmmGetAllocationOffset(ptrdiff_t *offset, void *ptr, cudaStream_t stream) {
  void *base   = (void *)0xffffffff;
  CUresult res = cuMemGetAddressRange((CUdeviceptr *)&base, nullptr, (CUdeviceptr)ptr);
  if (res != CUDA_SUCCESS) return RMM_ERROR_INVALID_ARGUMENT;
...
// with the stream.
rmmError_t
rmmGetInfo(size_t *freeSize, size_t *totalSize, cudaStream_t stream) {
  ...
  } catch (std::runtime_error const &) { return RMM_ERROR_CUDA_ERROR; }
  return RMM_SUCCESS;
}

// Write the memory event stats log to specified path/filename
rmmError_t
rmmWriteLog(const char *filename) {
  try {
    std::ofstream csv;
    csv.open(filename);
    rmm::Manager::getLogger().to_csv(csv);
  } catch (const std::ofstream::failure &e) { return RMM_ERROR_IO; }
  return RMM_SUCCESS;
}
...
rmmError_t
rmmGetLog(char *buffer, size_t buffer_size) {
  try {
    std::ostringstream csv;
    rmm::Manager::getLogger().to_csv(csv);
