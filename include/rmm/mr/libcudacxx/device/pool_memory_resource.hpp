...
class pool_memory_resource final
 ...
 public:
   ...
   * multiple of pool_memory_resource::allocation_alignment bytes.
   *
   * @param upstream_mr_view The memory_resource_view from which to allocate blocks for the pool.
 ...
 protected:
  ...
  thrust::optional<block_type> block_from_upstream(size_t size, cuda_stream_view stream)
  {
    ...
    try {
      void* p = upstream_mr_view_->allocate_async(size, stream);
   ...
   *
   */
  void release(cuda::stream_view stream = cuda::stream_view{})
  {
    lock_guard lock(this->get_mutex());

    for (auto b : upstream_blocks_)
      upstream_mr_view_->deallocate_async(b.pointer(), b.size(), stream);
  ...
  void print()
  {
    ...
    /*std::size_t free, total;
    std::tie(free, total) = upstream_mr_view_->get_mem_info(0);
