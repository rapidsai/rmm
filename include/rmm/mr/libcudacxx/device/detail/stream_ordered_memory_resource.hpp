...
class stream_ordered_memory_resource : public crtp<PoolResource>,
 ...
 protected:
   ...
   */
  virtual void* do_allocate_async(std::size_t bytes, cuda::stream_view stream)
  {
    ...
    auto stream_event = get_event(stream.get());
  ...
  virtual void* do_allocate_async(std::size_t bytes, std::size_t, cuda::stream_view stream) override
  {
    return do_allocate_async(bytes, stream);
  }

  virtual void do_deallocate_async(void* p,
                                   std::size_t bytes,
                                   std::size_t,
                                   cuda::stream_view stream) override
  {
    return do_deallocate_async(p, bytes, stream);
  }
   ...
   */
  virtual void do_deallocate_async(void* p, std::size_t bytes, cuda::stream_view stream)
  {
    lock_guard lock(mtx_);
    auto stream_event = get_event(stream.get());
