...
class pool_memory_resource final : public device_memory_resource {
 ...
 private:
  ...
  {
    block_t const alloc{b.pointer(), size, b.is_head()};
  ...
  void free_block(void* p, size_t size, cudaStream_t stream)
  {
    ...
    auto i = allocated_blocks_.find(static_cast<char*>(p));
  ...
  std::set<block_t, rmm::mr::detail::compare_blocks<block_t>> allocated_blocks_;
