...
class device_memory_resource {
 public:
   ...
   */
  void* allocate(std::size_t bytes, cuda_stream_view const& stream = cuda_stream_view{})
  {
   ...
   */
  void deallocate(void* p, std::size_t bytes, cuda_stream_view const& stream = cuda_stream_view{})
  {
   ...
   */
  std::pair<std::size_t, std::size_t> get_mem_info(cuda_stream_view const& stream) const
  {
 ...
 private:
   ...
   * @return void* Pointer to the newly allocated memory
   */
  virtual void* do_allocate(std::size_t bytes, cuda_stream_view const& stream) = 0;
   ...
   * @param stream Stream on which to perform deallocation
   */
  virtual void do_deallocate(void* p, std::size_t bytes, cuda_stream_view const& stream) = 0;
  ...
  virtual std::pair<std::size_t, std::size_t> do_get_mem_info(
    cuda_stream_view const& stream) const = 0;
};
}  // namespace mr
}  // namespace rmm
