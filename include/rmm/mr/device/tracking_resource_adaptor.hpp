...
class tracking_resource_adaptor final : public device_memory_resource {
 public:
  ...
  tracking_resource_adaptor(Upstream* upstream, bool capture_stacks = false)
    : capture_stacks_{capture_stacks}, allocated_bytes_{0}
  {
 ...
 * @param upstream device_async_resource_ref to the upstream resource
 * @return The new tracking resource adaptor
 */
template <typename Upstream>
tracking_resource_adaptor<Upstream> make_tracking_adaptor(device_async_resource_ref upstream)
{
