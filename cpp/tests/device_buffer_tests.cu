...
TYPED_TEST(DeviceBufferTest, MoveConstructor)
{
  rmm::device_buffer buff(this->size, rmm::cuda_stream_default, this->mr);
  auto* ptr              = buff.data();
  auto expected_size     = buff.size();
  auto expected_capacity = buff.capacity();
  auto expected_mr       = buff.memory_resource();
  auto expected_stream   = buff.stream();
  ...
  EXPECT_EQ(expected_size, buff_new.size());
  EXPECT_EQ(expected_capacity, buff_new.capacity());
  EXPECT_EQ(expected_stream, buff_new.stream());
  EXPECT_EQ(expected_mr, buff_new.memory_resource());
...
TYPED_TEST(DeviceBufferTest, MoveAssignmentToDefault)
{
  ...
  auto* ptr              = src.data();
  auto expected_size     = src.size();
  auto expected_capacity = src.capacity();
  auto expected_mr       = src.memory_resource();
  auto expected_stream   = src.stream();
  ...
  EXPECT_EQ(expected_capacity, dest.capacity());
  EXPECT_EQ(expected_stream, dest.stream());
  EXPECT_EQ(expected_mr, dest.memory_resource());
...
#pragma GCC diagnostic pop
  ...
  EXPECT_EQ(expected_capacity, buff.capacity());
  EXPECT_EQ(expected_stream, buff.stream());
  EXPECT_EQ(expected_mr, buff.memory_resource());
}
