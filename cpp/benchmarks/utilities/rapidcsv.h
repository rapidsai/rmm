/*
 * SPDX-FileCopyrightText: Copyright (C) 2017-2026 Kristofer Berggren. All rights reserved.
...
class Document {
 public:
                    ...
                    const std::string& pColumnName = std::string())
  {
    ...
    if (!pColumnName.empty()) { SetColumnName(pColumnIdx, pColumnName); }
                 ...
                 const std::string& pRowName = std::string())
  {
    ...
    if (!pRowName.empty()) { SetRowName(pRowIdx, pRowName); }
  ...
  void SetColumnName(size_t pColumnIdx, const std::string& pColumnName)
  {
    ...
    if (rowIdx >= mData.size()) { mData.resize(rowIdx + 1); }
    auto& row = mData[rowIdx];
    if (dataColumnIdx >= row.size()) { row.resize(dataColumnIdx + 1); }
  ...
  void SetRowName(size_t pRowIdx, const std::string& pRowName)
  {
    ...
    // increase table size if necessary:
    if (dataRowIdx >= mData.size()) { mData.resize(dataRowIdx + 1); }
 ...
 private:
      ...
      for (size_t i = 0; i < static_cast<size_t>(readLength); ++i) {
        if (buffer[i] == mSeparatorParams.mQuoteChar) {
          if (cell.empty() || (cell[0] == mSeparatorParams.mQuoteChar)) {
            ...
            if (std::all_of(
                  cell.begin(), firstQuote, [](unsigned char ch) { return isspace(ch); })) {
              quoted = !quoted;
            }
          }
          cell += buffer[i];
  ...
  void WriteCsv() const
  {
...
#endif
    {
      ...
      if (mHasUtf8BOM) { stream.write(s_Utf8BOM.data(), 3); }
