...
device_async_resource_ref indexed_pool_memory_resource_impl::get_upstream_resource() const noexcept
{
  return device_async_resource_ref{
    const_cast<cuda::mr::any_resource<cuda::mr::device_accessible>&>(upstream_mr_)};
}

std::size_t indexed_pool_memory_resource_impl::pool_size() const noexcept
{ return current_pool_size_; }

std::size_t indexed_pool_memory_resource_impl::get_maximum_allocation_size() const
{ return std::numeric_limits<std::size_t>::max(); }

indexed_pool_memory_resource_impl::block_type indexed_pool_memory_resource_impl::try_to_expand(
...
void indexed_pool_memory_resource_impl::initialize_pool(std::size_t initial_size,
...
indexed_pool_memory_resource_impl::block_type indexed_pool_memory_resource_impl::expand_pool(
  std::size_t size, [[maybe_unused]] free_list& blocks, cuda_stream_view stream)
{
  ...
  // block, reclaim entirely-free upstream blocks from every owner to free headroom under the cap.
...
void indexed_pool_memory_resource_impl::reclaim_free_blocks(std::size_t size,
                                                            cuda_stream_view stream)
{
  ...
  this->reclaim_free_upstream_blocks(
    stream, [&](free_list& blocks, cudaEvent_t owner_event, auto requester) {
...
indexed_pool_memory_resource_impl::block_from_upstream(std::size_t size, cuda_stream_view stream)
{
...
indexed_pool_memory_resource_impl::allocate_from_block(block_type const& block, std::size_t size)
{
...
indexed_pool_memory_resource_impl::prepared_allocation_tracking
indexed_pool_memory_resource_impl::prepare_allocation_tracking(block_type const& block)
{
  prepared_allocation_tracking prepared;
#ifdef RMM_POOL_TRACK_ALLOCATIONS
  std::set<block_type, compare_blocks<block_type>> staging;
  auto const iter = staging.insert(block).first;
  prepared.node   = staging.extract(iter);
#else
  (void)block;
#endif
  return prepared;
...
void indexed_pool_memory_resource_impl::commit_allocation_tracking(
  prepared_allocation_tracking&& prepared) noexcept
{
#ifdef RMM_POOL_TRACK_ALLOCATIONS
  auto const result = allocated_blocks_.insert(std::move(prepared.node));
  RMM_LOGGING_ASSERT(result.inserted);
#else
  (void)prepared;
#endif
}
...
void indexed_pool_memory_resource_impl::release()
{
...
std::pair<std::size_t, std::size_t> indexed_pool_memory_resource_impl::free_list_summary(
...
#ifdef RMM_DEBUG_PRINT
void indexed_pool_memory_resource_impl::print()
{
  lock_guard lock(this->get_mutex());
