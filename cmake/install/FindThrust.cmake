...
# Try to find Thrust
#
# Uses Thrust_ROOT in the cache variables or in the environment as a hint where
# to search
#
# IMPORTED Targets
# ^^^^^^^^^^^^^^^^
...
#
# ``Thrust_FOUND`` system has Thrust ``Thrust_INCLUDE_DIRS`` the Thrust include
# directories
...
string(REGEX MATCH "#define THRUST_VERSION ([0-9]*)" _match
             "${_version_header}")
...
if(Thrust_FOUND)
    # Create wrapper function to handle situation where we can't use a regular
    # IMPORTED INTERFACE target since that'll use -isystem, leading to the wrong
    # search order with nvcc
    function(thrust_create_target tgt)
        if(NOT TARGET ${tgt})
            add_library(thrust_internal INTERFACE)
            set_target_properties(
                thrust_internal PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                           "${Thrust_INCLUDE_DIRS}")
